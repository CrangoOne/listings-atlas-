...
const GH_CONSOLIDATE_PATH = "data/pending_consolidate.json";
...
export function buildConsolidateDispatcherPrompt(payload) {
  const lines = [
    "Consolidate Listings Atlas car data and publish KPIs to Pages.",
    "You are the consolidate dispatcher. Follow daq/jobs/CONSOLIDATE_AUTOMATION.md.",
    "Do not ask for confirmation.",
  ];
  if (payload.wave_id) {
    lines.push(`Wave filter: \`${payload.wave_id}\` (integrate worker PRs for this wave).`);
  } else if (payload.all_open) {
    lines.push("Integrate all open `cursor/daq-*` worker PRs (no wave filter).");
  }
  lines.push(
    "Run integrate_wave_csvs.py → consolidate_cardata.py → one integration PR → publish_atlas_assets."
  );
  return lines.join("\n");
}

export function buildConsolidatePayload({ waveId, allOpen = true }) {
  const requested_at = new Date().toISOString().replace(/\.\d{3}Z$/, "Z");
  const payload = {
    kind: "listings-atlas-consolidate",
    wave_id: waveId || null,
    all_open: Boolean(allOpen),
    requested_at,
  };
  const prompt = buildConsolidateDispatcherPrompt(payload);
  payload.prompt = prompt;
  payload.text = prompt;
  return payload;
}

/** Queue consolidate request via GitHub Contents API (CORS-ok from Pages). */
export async function queueConsolidateOnGitHub(payload, ghToken) {
  if (!ghToken) throw new Error("Save a GitHub token with Contents write on listings-atlas-.");
  const api = `https://api.github.com/repos/${GH_REPO}/contents/${GH_CONSOLIDATE_PATH}`;
  const headers = ghHeaders(ghToken);
  let sha;
  const getRes = await fetch(api, { headers, cache: "no-store" });
  if (getRes.ok) {
    ...
    const t = await getRes.text();
    throw new Error(`GitHub GET pending_consolidate ${getRes.status}: ${t.slice(0, 200)}`);
  }
  const body = {
    message: payload.wave_id
      ? `chore(consolidate): queue ${payload.wave_id}`
      : "chore(consolidate): queue all open worker PRs",
    content: utf8ToBase64(`${JSON.stringify(payload, null, 2)}\n`),
    branch: "main",
  };
  if (sha) body.sha = sha;
  ...
  const text = await putRes.text();
  if (!putRes.ok) {
    throw new Error(`GitHub consolidate queue ${putRes.status}: ${text.slice(0, 240)}`);
  }
  return { status: putRes.status, via: "github" };
}

export async function postConsolidateDirect(payload, { url, token }) {
  if (!url) throw new Error("Save the automation webhook URL first.");
  if (!token) throw new Error("Save the automation Bearer token first.");
  if (!looksLikeWebhookUrl(url)) {
    throw new Error(
...
/**
 * Prefer GitHub queue (CORS-safe); fall back to direct webhook.
 */
export async function postConsolidateRequest(payload, settings) {
  const { url, token, ghToken } = settings;
  if (ghToken) {
    return queueConsolidateOnGitHub(payload, ghToken);
  }
  try {
    return await postConsolidateDirect(payload, { url, token });
  } catch (err) {
    const msg = err?.message || String(err);
    if (/failed to fetch|networkerror|cors/i.test(msg)) {
      throw new Error(
        "Consolidate failed: GitHub Pages cannot call the Cursor webhook (browser CORS). " +
          "Paste your listings-atlas- GitHub PAT (Contents: Read and write) in settings, " +
          "add Forward consolidate workflow + webhook secrets on listings-atlas-, then try again."
      );
    }
    throw new Error(msg.startsWith("Consolidate failed") ? msg : `Consolidate failed: ${msg}`);
  }
}
